...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = np.sqrt(np.abs(1-np.sum(np.square(mu_orth))))*((e_w1B-mu_B)/np.sum(np.abs((e_w1-mu_orth)-mu_B)))
    corrected_e_w2B = np.sqrt(np.abs(1-np.sum(np.square(mu_orth))))*((e_w2B-mu_B)/np.sum(np.abs((e_w2-mu_orth)-mu_B)))
...
```
    ...
    cosine_similarity(e1, gender) =  -0.185463501079
    cosine_similarity(e2, gender) =  0.185463501079
