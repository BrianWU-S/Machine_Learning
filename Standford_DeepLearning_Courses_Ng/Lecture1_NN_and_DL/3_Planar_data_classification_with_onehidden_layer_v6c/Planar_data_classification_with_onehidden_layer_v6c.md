...
```python
# GRADED FUNCTION: layer_sizes   --> == layer's unit number == layer ouput's row shape
...
def layer_sizes(X, Y):
    ...
    Returns:
    n_x -- the size of the input layer  (input layer is actually the input, not a layer, so we call it layer 0)
    n_h -- the size of the hidden layer  (hidden layer is the real first layer, it's layer 1)
    n_y -- the size of the output layer
    """
    ### START CODE HERE ### (≈ 3 lines of code)
    n_x =  np.shape(X)[0]   # n_x is equal to the feature size of input X
    n_h = 4
    n_y =  np.shape(Y)[0]   # size of output layer is equal to the feature size of output Y
    ### END CODE HERE ###
    return (n_x, n_h, n_y)
```
...
```
    ...
    <matplotlib.text.Text at 0x7f3c63908860>
